...
const AddSpot = ({ classes, history }) => {
  ...
  const [addMutation, { data: addData, loading: addLoading, error: addError }] =
    useMutation(
      gql`
        ${ADD_SPOT}
      `,
  ...
  const seasonStartChange = (event) => {
    ...
    setState({
      ...state,
      waveSeasonStartLabel: newValue,
      waveseasonstart: Number(waveSeasonStartValue),
    });
  };
  ...
  const seasonEndChange = (event) => {
    ...
    setState({
      ...state,
      waveSeasonEndLabel: newValue,
      waveseasonend: Number(waveSeasonEndValue),
    });
  };
  ...
  const handleSubmit = (event) => {
    ...
    if (missingProperties.length > 0) {
      alert(
        `Missing the following value(s): ${missingProperties.join(
          ","
    ...
    if (missingLocations.length > 0) {
      alert(
        `Missing the following value(s): ${missingLocations.join(
          ","
        )}\n Please add them and try submitting the form again`
      );
      return null;
    ...
    Object.entries(ratingObject).forEach(([key, value]) => {
      if (!value) {
        missingRatings.push(key);
      }
    });
    if (missingRatings.length > 0) {
      alert(
        `Missing the following value(s): ${missingRatings.join(
          ","
        )}\n Please add them and try submitting the form again`
      );
      return null;
  ...
  if (addError) {
    return (
      <p>
        something went wrong with your submission. Please refresh the page and
        try again
      </p>
    );
  }
  if (
    addData &&
    !(ratingLoading || locationLoading) &&
    !(ratingData || locationData)
  ) {
    let locationId = addData.insert_Waves_one.id;
    let waveId = addData.insert_Waves_one.id;
    let userId = addData.insert_Waves_one.createdby;
  ...
  if (locationData && ratingData && addData) {
    history.push(`/Wave/${addData.insert_Waves_one.id}`);
  }
  return (
    <div>
          ...
          <Grid container justify="space-between">
            <Grid item xs={12} md={5}>
              <TextField
                ...
                margin="normal"
                name="windangleone"
                value={state.windangleone}
                onChange={windAngleOne}
              />
            </Grid>
            <Grid item xs={12} md={5}>
              <TextField
                ...
                margin="normal"
                name="windangletwo"
                value={state.windangletwo}
                onChange={windAngleTwo}
              />
            </Grid>
            <Grid item xs={12} md={5}>
              <TextField
